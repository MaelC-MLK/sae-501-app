...
export default function UnregisterPage({ params }: { params: { token: string, id: string } }) {
    ...
    useEffect(() => {
        ...
        const verifyToken = async () => {
            try {
                const res = await fetch(`${process.env.API_BASE_URL}/api/user/email/unregister/${token}/${id}`);
                if (res.ok) {
                    const data = await res.json();
                    if (data.message === 'Utilisateur non inscrit') {
                        setStatus('alreadyVerified');
                        setMessage(data.message);
                    } else {
                        setStatus('success');
                        setMessage('Votre désinscription a été validée avec succès !');
                    }
                } else {
                    const data = await res.json();
                    setStatus('error');
