...
export function CardEvent({ event }: { event: EventProps }) {
    ...
    const limit = event.limit ? parseInt(event.limit, 10) : 0;
    const fillPercentage = limit > 0 ? (event.users.length / limit) * 100 : 0;

    let badgeVariant = "secondary";
    if (fillPercentage >= 75) {
        badgeVariant = "destructive"; // Red
    } else if (fillPercentage >= 50) {
        badgeVariant = "warning"; // Yellow
    } else {
    ...
    return (
        <Link href={`/event/${event.id}`} className="border-2 rounded-xl overflow-hidden p-2 w-96 group bg-background relative h-fit">
            <Badge variant={badgeVariant} className={`absolute top-4 right-4 z-20 gap-1`}>
